...
const DeactivatedUsers: React.FC = () => {
  ...
  const { users, reactivateUser } = useStore();
  const [searchTerm, setSearchTerm] = useState('');
  const [reactivateConfirm, setReactivateConfirm] = useState<{ isOpen: boolean; id: string }>({ isOpen: false, id: '' });
  ...
  const handleReactivate = () => {
    reactivateUser(reactivateConfirm.id);
    setReactivateConfirm({ isOpen: false, id: '' });
  };
  ...
  return (
    <div className="space-y-8">
      <ConfirmDialog
        ...
        onClose={() => setReactivateConfirm({ isOpen: false, id: '' })}
      ...
      <div className="flex flex-col gap-1">
        ...
        <p className="text-sm text-gray-500">View and manage deactivated user accounts</p>
      </div>
            ...
            {filteredUsers.length === 0 ? (
              ...
              filteredUsers.map((user, idx) => (
                <tr key={user.id} className="hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors">
                  ...
                  <td className="px-6 py-4 text-sm text-gray-500 dark:text-gray-400">{user.designation}</td>
                  <td className="px-6 py-4">
                    <div className="flex items-center justify-center">
                      <button
                        onClick={() => setReactivateConfirm({ isOpen: true, id: user.id })}
